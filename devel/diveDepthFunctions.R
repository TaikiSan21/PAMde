...
export_diveDepthNEFSC <- function(x, outDir=NULL, file=NULL, wavFolder, effort, clipLength=.03, soundSpeed=1500, depthSensAcc=1, progress=TRUE) {
  ddTable <- createDiveDepthTable(x, effort=effort, soundSpeed=soundSpeed, depthSensAcc=depthSensAcc, wavFolder=wavFolder)
  if(is.null(outDir)) {
    outDir <- wavFolder
  }
  clipMat <- plotEchoSummary(wavFolder, outDir=outDir, ncol=5, nrow=100,
  ...
  list(wavMat = clipMat, table = ddTable, file=file.path(outDir, file))
}

# plot binary wav clips
plotBinaryWavClips <- function(x, wavFolder, clipLength, channel=1) {
  # read UID from wav folder
  wavFiles <- list.files(dir, pattern='\\.wav$', full.names = TRUE)
  wavUIDs <- parseWavClipName(wavFiles, mode='UID')
  # read binary for UIDs
  binData <- getBinaryData(x, wavUIDs, type='click')
  sr <- readWave(wavFiles[1], header=TRUE)$sample.rate
  for(i in seq_along(binData)) {
    
  }
  # plot wavs for a channel - fix to clipLength using sr of one wav file
}
#' @importFrom readxl read_excel
# get effort, match bft to wave height, then set key for matching later
readEffortNEFSC <- function(x) {
  if(is.character(x)) {
    if(grepl('\\.xls', x)) {
      effort <- read_excel(x)
      ...
      effort <- select(effort, c('BEAUFORT', 'BEGDATETIMEGMT'))
      effort[['BEGDATETIMEGMT']] <- PAMpal:::parseUTC(effort[['BEGDATETIMEGMT']], format='%m/%d/%Y %H:%M:%S')
    }
  }
  effort <- rename(effort, 'UTC' = 'BEGDATETIMEGMT', 'Beaufort'='BEAUFORT')
  # effort <- sample_n(effort, 20)
  setDT(effort)
  setkeyv(effort, 'Beaufort')
  bft <- PAMpal:::ppVars()$bftHeight
  effort <- bft[effort, roll=-Inf]
  setkeyv(effort, 'UTC')
  effort
}
####
...
# mcmc localisation
####
createDiveDepthTable <- function(x, effort, soundSpeed=1500, depthSensAcc = 1, wavFolder) {
  clickData <- getClickData(x) %>% 
    select(c('UID', 'UTC', 'species', 'Latitude', 'Longitude', 'depth', 'eventId')) %>% 
    distinct()
  tmData <- bind_rows(lapply(events(x), function(e) {
    c(id = id(e),
      localizations(e)$PGTargetMotion[c('TMLatitude1', 'TMLongitude1', 
                                        'TMPerpendicularDistance1', 'TMPerpendicularDistanceError1')])
  }))
  ...
  clickData$RadialDistM <- distGeo(matrix(c(clickData$Longitude, clickData$Latitude), ncol=2),
                                   matrix(c(clickData$TMLongitude1, clickData$TMLatitude1), ncol=2))
  clickData <- select(clickData,
                      UID,
                      EventId = eventId,
                      GroupLat = TMLatitude1,
                      GroupLon = TMLongitude1,
                      ShipLat = Latitude,
                      ShipLon = Longitude,
                      SlantRange = TMPerpendicularDistance1,
                      SlantRangeErr = TMPerpendicularDistanceError1,
                      Species = species,
                      ClickTime = UTC,
                      ArrayDepth = depth,
                      RadialDistM) %>% 
  ...
  setDT(clickData)
  setkeyv(clickData, 'ClickTime')
  effort <- readEffortNEFSC(effort)
  # this currently matches each detection time to the previous effort start time
  clickData <- effort[clickData, roll=12*60*60]
  setDF(clickData)
  clickData <- rename(clickData, 'Beaufort' = 'bftMax', 'Waveheight' = 'waveHeight')
  ...
  wavFiles <- list.files(wavFolder, pattern= '\\.wav')
  wavDf <- data.frame(Filename = wavFiles, UID = parseWavClipName(wavFiles, 'UID'))
...
plotEchoSummary <- function(dir, outDir='.',
  ...
  for(w in seq_along(wavFiles)) {
    ...
    if(!is.null(specFilter)) {
      from <- specFilter$from
      to <- specFilter$to
    }
    
    thisWav <- seewave::bwfilter(thisWav, from=from, to=to)[,1]
    wavMat[, w] <- thisWav
  }
  if(progress) {
    cat('\n')
...
parseArrayDepth <- function(depth, wavName, error=FALSE) {
  ...
  if(is.AcousticStudy(depth)) {
    ...
    thisUID <- parseWavClipName(wavName, 'UID')
    return(unique(clickData$depth[clickData$UID == thisUID]))
  }
  NA
}
